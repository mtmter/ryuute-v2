...
`LS8H_TRANSIT_API_URL`（既定 `https://api.transit.ls8h.com/api/v1/plan`）を30秒timeoutで
呼び出します。座標を `geo:<緯度>,<経度>` として `from` と `to` に送り、`date`（`YYYYMMDD`）、
`time`、`type`（`departure` または `arrival`）、`numItineraries=5` を送ります。徒歩だけの
候補と公共交通を含む候補が混在する場合は、公共交通を含む候補を優先します。
