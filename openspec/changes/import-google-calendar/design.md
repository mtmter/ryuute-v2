...
3. Mirror imported instances into `events` with a deterministic local document ID derived from provider, calendar ID and Google event ID. Store source-owned fields under `external`, local augmentations in existing top-level PlanRail fields, and `source_status` for active/cancelled.
